...
(function () {
  ...
  const EMOTIONAL_QUOTES = {
    ...
    // ============================================
    // 状态四：高压预警/崩溃 (Critical DSI 86-100)
    // ============================================
    CRITICAL_DSI: [
      "🚨 警报！系统过载！立即执行「什么都不做」程序！",
      "😱 别点了别点了！鼠标要被你点疼啦！",
      "🔥 我要着火了！快停下来救救我！",
      "🛑 强制命令：立刻离开屏幕 1 分钟！",
      "💔 心碎了，你的状态让我好担心",
  ...
  function getRandomQuote(dsi) {
    ...
    if (hour >= 6 && hour < 12) {
      ...
      // 午后 (12:00 - 17:00)
      pool = EMOTIONAL_QUOTES.AFTERNOON;
    } else if (hour >= 18 && hour < 22) {
      // 夜晚 (18:00 - 22:00)
      pool = EMOTIONAL_QUOTES.NIGHT;
    } else if (hour >= 22 || hour < 6) {
      // 深夜 (22:00 - 05:00)
      pool = EMOTIONAL_QUOTES.LATE_NIGHT;
    } else {
      // 其他时间按DSI选择（状态优先级）
      if (dsi <= 35) {
        pool = EMOTIONAL_QUOTES.LOW_DSI; // 森之静谧
      } else if (dsi <= 65) {
        pool = EMOTIONAL_QUOTES.MEDIUM_DSI; // 轻度压力
      } else if (dsi <= 85) {
        pool = EMOTIONAL_QUOTES.HIGH_DSI; // 压力上升
      } else {
        pool = EMOTIONAL_QUOTES.CRITICAL_DSI; // 高压预警
      }
    }
  ...
  class MindFlowHUD {
    constructor() {
      ...
      this.dsi = 0;
      this.level = 0;
      this.lastLevel = 0; // 记录上一次的等级，用于检测突变
    ...
    async showSpeechBubbleWithAI() {
      ...
      try {
        // 2. 强制请求 AI（跳过缓存检查，确保获取最新内容）
        const greeting = await this.fetchContextGreeting(this.dsi, true); // skipCache = true
        if (greeting && quoteEl) {
        ...
        console.warn('[MindFlow] AI 问候获取失败，使用本地语录:', e);
        if (quoteEl) quoteEl.textContent = getRandomQuote(this.dsi);
      }
     ...
     * @param {boolean} skipCache - 是否跳过缓存（用于 DSI 突变等紧急情况）
     * @returns {Promise<string|null>} - 问候语或 null
     */
    async fetchContextGreeting(dsi, skipCache = false) {
      const cacheKey = `context_greeting_${window.location.href}`;

      try {
        // 检查缓存（如果 skipCache 为 true，则跳过）
        if (!skipCache) {
          const cached = sessionStorage.getItem(cacheKey);
          if (cached) {
            const cacheData = JSON.parse(cached);
          ...
          console.log('[Context Greeting] 跳过缓存，强制获取最新内容');
        }
      ...
      setInterval(async () => {
        try {
          ...
          if (response && response.success) {
            ...
            // ✅ 新增：检测压力等级突变，触发 AI 介入
            // 条件：等级上升，且当前等级至少为 1 (轻度压力)，防止在低压区频繁打扰
            if (calculatedLevel > this.lastLevel && calculatedLevel >= 1) {
              console.log(`[MindFlow] 📈 压力等级上升 (Lv${this.lastLevel}->Lv${calculatedLevel})，触发 AI 关怀`);
              
              // 强制显示气泡，80% 概率使用 AI，20% 使用本地急救语录
              if (Math.random() < 0.8) {
                this.showSpeechBubbleWithAI();
              } else {
                // 使用本地语录（快速响应）
                const quoteEl = document.getElementById('hud-bubble-quote');
                if (quoteEl) {
            ...
            // 更新记录
            this.lastLevel = calculatedLevel;
    ...
    startIdleChatter() {
      ...
      const scheduleNext = () => {
        // 提高频率：缩短间隔到 30-90 秒（原来 60-180 秒）
        // 这样平均每 1 分钟就会有一次触发机会
        const delay = (Math.random() * 60 + 30) * 1000; // 30-90秒
        ...
        setTimeout(() => {
          ...
          if (!this.isBubbleVisible && this.dsi < 90) {
            ...
            // 提高触发概率到 70%（原来 50%），增加存在感
            if (Math.random() > 0.3) {
              // 根据 DSI 值决定内容策略：
              // - 低 DSI (<=30): 主要使用本地语料库（90%），偶尔 AI（10%）
              // - 中 DSI (30-60): 混合使用（70% 本地，30% AI）
              // - 高 DSI (>60): 优先 AI（50% AI，50% 本地）
              let shouldUseAI = false;
              if (this.dsi <= 30) {
                // 低压力时主要用语料库，偶尔 AI 增加新鲜感
                shouldUseAI = Math.random() < 0.1; // 10% 概率使用 AI
              } else if (this.dsi <= 60) {
                // 中等压力时混合使用
                shouldUseAI = Math.random() < 0.3; // 30% 概率使用 AI
              } else {
                // 高压力时优先 AI，但也要有本地兜底
                shouldUseAI = Math.random() < 0.5; // 50% 概率使用 AI
              }
